...
export async function updateSupportedModelsFile(models: APIResponse) {
  ...
  await updateChatModels(await readCodegenFile(PATH), PATH, models);
  await updateEmbeddingModels(await readCodegenFile(PATH), PATH, models);
  await updateTextgenModels(await readCodegenFile(PATH), PATH, models);
}
...
async function updateChatModels(
  ...
  const chatModels = models.resources
    .filter(
      (resource) =>
    ...
    .flatMap((resource) => {
      ...
      return [
        `| \`${resource.model_id}\` | ${resource.provider} | ${fcSo} | ${fcSo} |`,
      ];
    })
    .join('\n');
...
async function updateEmbeddingModels(
  ...
  const embeddingModels = models.resources
    ...
    .flatMap(
      (resource) =>
        `| \`${resource.model_id}\` | ${resource.provider} | ${resource.model_limits?.max_sequence_length ?? 'N/A'} | ${resource.model_limits?.embedding_dimension ?? 'N/A'} |`,
    )
    .join('\n');
...
async function updateTextgenModels(
  ...
  const textGenModels = models.resources
    .filter(
      (resource) =>
        !resource.lifecycle.some((lifecycle) => lifecycle.id === 'deprecated'),
    )
    ...
    .flatMap(
      (resource) => `| \`${resource.model_id}\` | ${resource.provider} |`,
    )
    .join('\n');
  ...
  const settingsAutogen = createCodegen({
    ...
    content: ['| ID | Provider |', '|----|----------|', textGenModels],
    append: '\n',
  });
